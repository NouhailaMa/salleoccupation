...
const getSalle=async (req,res)=>{
    try {
        ...
        if(salle==null){
            return res.status(404).json({msg:`Il n'y a pas de salle avec cette id:${salleID}`});
        }
...
const deleteSalle=async (req,res)=>{
    try {
        ...
        const salle =await Salle.findOneAndDelete({_id:salleID}).populate('bloc');
        if(salle==null){
            return res.status(404).json({msg:`Il n'y a pas de salle avec cette id:${salleID}`});
        }
        res.status(201).json({salle,"ocuupations supprimées":occupation.deletedCount});
    } catch (error) {
        res.status(500).json({msg:error});
    } 
}
