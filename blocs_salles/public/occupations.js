...
const showOccupations = async () => {
    ...
    try {
      const {
        data: occupations,
      } = await axios.get('/api/occupations')
      if (occupations.length < 1) {
        tasksDOM.innerHTML = '<h5 class="empty-list"></h5>'
        loadingDOM.style.visibility = 'hidden'
        return
      }
      const allOccupations = occupations
        .map((occupation) => {
          const { _id: occupationID,salle,creneau,date } = occupation
          return `
          <tr>
              <td>${salle.nom}</td>
              ...
              <td>${date}</td>
              <td>
              <button type="button" class="btnu btn btn-outline-success" data-id="${occupationID}" data-salleid="${salle._id}" data-creneauid="${creneau._id}">
              <i class="fas fa-pen"></i>
              </button>
              <button type="button" class="btnd btn btn-outline-danger" data-id="${occupationID}">
 ...
 const showSalles = async () => {
    ...
    try {
      ...
      if (salles.length < 1) {
        tasksDOM.innerHTML = '<h5 class="empty-list">No Occupations in your list</h5>'
        loadingDOM.style.visibility = 'hidden'
        return
      }
  ...
  showSalles()
   ...
   const showCreneaux=async ()=>{
    ...
    try {
        ...
        const allCreneaux=creneaux.map((creneau)=>{
...
formDOM.addEventListener('submit', async (e) => {
    ...
    try {
        if(submitDOM.value=="Ajouter une occupation"){
            const {data:msg}=await axios.post('/api/occupations', { salle:salle,creneau:creneau })
            ...
            await axios.patch(`/api/occupations/${id}`, { salle:salle,creneau:creneau })
